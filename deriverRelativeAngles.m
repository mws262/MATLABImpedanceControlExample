%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Derive double pendulum dynamics %%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Derive double pendulum equations of motion, write to Thdotdot1 and
%Thdotdot2 matlab function files. THIS VERSION DERIVES IT WITH THE ELBOW
%ANGLE RELATIVE TO THE UPPER ARM ANGLE.

%Parameters symbolically.
syms m1 m2 I1 I2 g l1 l2 d1 d2 th1 th2 thdot1 thdot2 thdotdot1 thdotdot2 er1 er2 eth1 eth2 T1 T2 real

%Unit vectors, cartesian
i = [1 0 0]';
j = [0 1 0]';
k = [0 0 1]';

%Rotating reference frames
er1 = [-sin(th1), cos(th1), 0]'; %Just changed it so 0 is upright.
er2 = [-sin(th2+th1), cos(th2+th1), 0]';

eth1 = [-cos(th1),-sin(th1),0]';
eth2 = [-cos(th2+th1),-sin(th2+th1),0]';

%Vectors to significant points
ra_c1 = d1*er1; %A is fixed point, B is the elbow, c1 and c2 are COMs, e end effector
rb_c2 = d2*er2;
rb_e = l2*er2;
ra_b = l1*er1;
ra_c2 = ra_b + rb_c2;
ra_e = ra_b + rb_e;

matlabFunction(ra_e, 'file', 'ForwardKin');

%Velocities
Vc1 = d1*thdot1*eth1;
VB = l1*thdot1*eth1;
Vc2 = VB + d2*(thdot2+thdot1)*eth2;
Ve = VB + l2*(thdot2+thdot1)*eth2;

%Accelerations
Ac1 = d1*thdotdot1*eth1 - d1*thdot1^2*er1;
AB = l1*thdotdot1*eth1 - l1*thdot1^2*er1;
Ac2 = d2*(thdotdot2+thdotdot1)*eth2 - d2*(thdot1 + thdot2)^2*er2  + AB;

%Force at end effector
syms Fdx Fdy real
Fd = [Fdx, Fdy, 0]';


%AMB for just link 2:
M_B = cross(rb_c2,-m2*g*j)+T2*k     + cross(rb_e,Fd); %Last term is force at end effector.
Hdot2 = I2*(thdotdot2+thdotdot1)*k + cross(rb_c2, m2*Ac2);

eqn2forthdotdot2 = solve(dot(Hdot2 - M_B,k),thdotdot2);

%AMB for whole thing:
M_A = cross(ra_c2,-m2*g*j) + cross(ra_c1,-m1*g*j) + T1*k + cross(ra_e,Fd); %Gravity for both, plus a control torque. Last term is force at end effector
Hdot1 = I2*(thdotdot2+thdotdot1)*k + cross(ra_c2, m2*Ac2) + I1*thdotdot1*k + cross(ra_c1, m1*Ac1);

eqn1forthdotdot1 = solve(dot(Hdot1 - M_A,k),thdotdot1);

%One equation for thdotdot1, one for thdotdot2.
eqn2 = simplify(solve(subs(eqn2forthdotdot2, thdotdot1, eqn1forthdotdot1)-thdotdot2,thdotdot2));
eqn1 = simplify(solve(subs(eqn1forthdotdot1, thdotdot2, eqn2forthdotdot2)-thdotdot1,thdotdot1));

%Create matlab functions for thdotdot1 and 2:
matlabFunction(eqn1, 'file', 'Thdotdot1');
matlabFunction(eqn2, 'file', 'Thdotdot2');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Gravity Compensation %%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

T2Eq = simplify(solve((solve(eqn1,T1)-solve(eqn2,T1)),T2));

T1Eq = simplify(subs(solve(eqn1,T1),T2,T2Eq));

matlabFunction(T1Eq, 'file', 'GravityCompT1');
matlabFunction(T2Eq, 'file', 'GravityCompT2');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Impedance control?   %%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%Jacobian relating end effector velocity to joint space vel
% ie Ve = J*qv

J = jacobian(Ve,{thdot1 thdot2});

syms Kp Kd xt yt xdott ydott real

zt = [xt yt 0 ]'; %Trajectory tracked
ztdot = [xdott ydott 0]'; %velocity tracked

Ta = J'*(Kp*(zt - ra_e) + Kd*(ztdot - J*[thdot1 thdot2]'));

matlabFunction(Ta, 'file', 'ImpedenceControl');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Energy eqns and control %%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

syms k1 k2 real

PE = g*dot(ra_c2,j)*m2 + g*dot(ra_c1,j)*m1;
KE = 1/2*I1*thdot1^2 + 1/2*I2*(thdot2+thdot1)^2 + 1/2*m1*dot(Vc1,Vc1) + 1/2*m2*dot(Vc2,Vc2);

Etot = PE + KE;

% %Desired energy:
% Edes = g*(m1*d1 + m2*(d2+l1)); %Max potential energy
% 
% Er = Etot-Edes; %error
% 
% Edot = diff(Etot,th1)*thdot1 + diff(Etot,th2)*thdot2 + diff(Etot,thdot1)*thdotdot1 + diff(Etot,thdot2)*thdotdot2;
% 
% Edotfull = subs(subs(Edot,thdotdot1,eqn1),thdotdot2,eqn2); %With thdotdot1 and 2 substituted in.
% 
% dEdT = diff(subs(Edotfull,T1,0),T2);
% 
% fdbk = -k1*dEdT*Er;
% 
% matlabFunction(fdbk, 'file', 'SwingUpTorque');

matlabFunction(Etot, 'file', 'TotEnergy');


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%% Linearized model and LQR control %%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% %For thdotdot1 equation
% diff1 = diff(eqn1,th1);
% diff2 = diff(eqn1,th2);
% diff3 = diff(eqn1,thdot1);
% diff4 = diff(eqn1,thdot2);
% 
% %Full expression linearized for equation 1(thdotdot1)
% %f~f(0) + dfda(0)*a +...
% linthdotdot1 = subs(eqn1,{th1,th2,thdot1,thdot2},{0,0,0,0}) +...
%     subs(diff1,{th1,th2,thdot1,thdot2},{0,0,0,0})*th1 + subs(diff2,{th1,th2,thdot1,thdot2},{0,0,0,0})*th2 +...
%     subs(diff3,{th1,th2,thdot1,thdot2},{0,0,0,0})*thdot1 + subs(diff4,{th1,th2,thdot1,thdot2},{0,0,0,0})*thdot2;
% 
% %For thdotdot2 equation
% diff5 = diff(eqn2,th1);
% diff6 = diff(eqn2,th2);
% diff7 = diff(eqn2,thdot1);
% diff8 = diff(eqn2,thdot2);
% 
% linthdotdot2 = subs(eqn2,{th1,th2,thdot1,thdot2},{0,0,0,0}) +...
%     subs(diff5,{th1,th2,thdot1,thdot2},{0,0,0,0})*th1 + subs(diff6,{th1,th2,thdot1,thdot2},{0,0,0,0})*th2 +...
%     subs(diff7,{th1,th2,thdot1,thdot2},{0,0,0,0})*thdot1 + subs(diff8,{th1,th2,thdot1,thdot2},{0,0,0,0})*thdot2;
% 
% 
% %Linearized model state space Xdot = AX + Bu
% A = [0 1 0 0
%     diff(linthdotdot1,th1) diff(linthdotdot1,thdot1) diff(linthdotdot1,th2) diff(linthdotdot1,thdot2)
%     0 0 0 1
%     diff(linthdotdot2,th1) diff(linthdotdot2,thdot1) diff(linthdotdot2,th2) diff(linthdotdot2,thdot2)];
% 
% %We're only controlling T2, the elbow torque
% B = [0
%     diff(linthdotdot1,T2)
%     0
%     diff(linthdotdot2,T2)];
% 
% %Y = CX + D
% C = [1 0 1 0];
% D = 0;
% 
% %Physical parameters -- for substituting actual system numbers
% g = p.g;
% m1 = p.m1;
% m2 = p.m2;
% l1 = p.l1;
% l2 = p.l2;
% d1 = l1/2;
% d2 = l2/2;
% I1 = 1/12*m1*l1^2;
% I2 = 1/12*m2*l2^2;
% 
% %Using system parameters numerically
% Anum = eval(A);
% Bnum = eval(B);
%     
% sys = ss(Anum,Bnum,C,D); %Original linearized model w/o control
% 
% Con = ctrb(sys);
% % Q = Con'*Con;
% Q = eye(4,4);
% Q = [1 0 0 0; 0 5 0 0; 0 0 1 0; 0 0 0 5];
% R = 1;
% 
% LQRGain = lqr(sys,Q,R);
% p.LQRGain = LQRGain; %Assign it in the struct that will be passed to the dynamics integration also.


